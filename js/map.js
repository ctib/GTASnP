...
function createPopupContent(g) {
  const stilBadge = g.stil
    ? `<span class="stil-badge modul-27210">${esc(g.stil)}</span><br>`
    : '';
...
function buildSidebarHTML(g) {
  ...
  // Stil-Badge
  if (g.stil) {
    parts.push(`<span class="stil-badge modul-27210">${esc(g.stil)}</span>`);
  }
  ...
  parts.push(`
    <div class="sidebar-section">
      ...
      <div class="sidebar-modul">
        <span class="sidebar-modul-dot modul-27210"></span>
        Nachhaltiges Planen (27210)${g.semester ? ` &middot; ${g.semester}` : ''}
      </div>
    </div>
  `);
...
async function loadGebaeude() {
  ...
  data.forEach(g => {
    const marker = L.marker([g.lat, g.lng], { icon: defaultIcon })
      .addTo(layerNachhaltig)
